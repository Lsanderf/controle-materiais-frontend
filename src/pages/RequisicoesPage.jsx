...
const emptyItem = () => ({ descricao: '', quantidade: '' });
...
export default function RequisicoesPage() {
  ...
  const loader = useCallback(() => {
    ...
    return Promise.all([requisicaoService.list(), usuarioService.listEncarregados(), contratoService.list()]);
  }, [isGerente]);
  const { data, loading, error, reload } = useResource(loader, [loader]);
  const [requisicoes = [], encarregados = [], contratos = []] = isGerente ? data ?? [] : [data ?? []];
  ...
  async function submit(event) {
    ...
    if (saving) return;
    const itens = form.itens.map((item) => ({ descricao: item.descricao.trim(), quantidade: Number(item.quantidade) }));
    if (!form.encarregadoDestinatarioId || !form.contratoId || itens.length === 0 || itens.some((item) => !item.descricao || !Number.isInteger(item.quantidade) || item.quantidade < 1)) {
      setActionError(new Error('Selecione destinatário, contrato e informe descrição e quantidade válida para cada material.'));
      return;
    }
  ...
  return <div className="page-stack"><header className="page-heading"><div><span className="eyebrow">{isEncarregado ? 'Recebimento' : 'Solicitações'}</span><h1>{isEncarregado ? 'Requisições recebidas' : isGerente ? 'Minhas requisições' : 'Requisições'}</h1><p>{isEncarregado ? 'Confira e conclua as solicitações enviadas para você.' : 'Acompanhe solicitações de retirada e devolução sem alterar o estoque.'}</p></div>{isGerente && <button className="button button-primary" type="button" onClick={() => setFormOpen((open) => !open)}>+ Nova requisição</button>}</header><SuccessMessage>{success}</SuccessMessage><ErrorMessage error={actionError} />
    {isGerente && formOpen && <form className="content-card form-card" onSubmit={submit}><h2>Nova requisição</h2><div className="form-grid"><label className="field"><span>Encarregado destinatário</span><select value={form.encarregadoDestinatarioId} onChange={(event) => updateForm('encarregadoDestinatarioId', event.target.value)} required><option value="">Selecione</option>{encarregados.filter((item) => item.ativo).map((item) => <option key={item.id} value={item.id}>{item.nome}</option>)}</select></label><div className="field"><span>Contrato</span><div className="requisicao-contract-field"><select value={form.contratoId} onChange={(event) => updateForm('contratoId', event.target.value)} required><option value="">Selecione</option>{contratos.filter((item) => item.ativo).map((item) => <option key={item.id} value={item.id}>{item.nome}</option>)}</select><Link className="button button-secondary" to="/contratos/novo" state={{ from: '/requisicoes' }}>+ Criar contrato</Link></div></div><label className="field"><span>Tipo</span><select value={form.tipo} onChange={(event) => updateForm('tipo', event.target.value)}><option value="RETIRADA">Retirada</option><option value="DEVOLUCAO">Devolução</option></select></label></div><fieldset className="requisicao-items"><legend>Materiais solicitados</legend>{form.itens.map((item, index) => <div className="requisicao-item-row" key={index}><label className="field"><span>Nome/descrição do material</span><input value={item.descricao} maxLength="255" onChange={(event) => updateItem(index, 'descricao', event.target.value)} required /></label><label className="field"><span>Quantidade</span><input type="number" min="1" max="10000" value={item.quantidade} onChange={(event) => updateItem(index, 'quantidade', event.target.value)} required /></label><button className="text-button danger requisicao-remove" type="button" onClick={() => removeItem(index)}>Remover</button></div>)}</fieldset><button className="text-button" type="button" onClick={addItem}>+ Criar material</button><label className="field"><span>Observação (opcional)</span><textarea maxLength="1000" value={form.observacao} onChange={(event) => updateForm('observacao', event.target.value)} /></label><div className="form-actions"><button className="button button-secondary" type="button" onClick={() => setFormOpen(false)}>Cancelar</button><button className="button button-primary" disabled={saving}>{saving ? 'Enviando...' : 'Enviar requisição'}</button></div></form>}
